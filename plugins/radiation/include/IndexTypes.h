...
struct PrimitiveUUID {
    ...
    explicit operator uint() const { return value; }

    /// Equality comparison
    constexpr bool operator==(const PrimitiveUUID& other) const { return value == other.value; }
    constexpr bool operator!=(const PrimitiveUUID& other) const { return value != other.value; }
};
...
struct ArrayPosition {
    ...
    /// Equality comparison
    constexpr bool operator==(const ArrayPosition& other) const { return value == other.value; }
    constexpr bool operator!=(const ArrayPosition& other) const { return value != other.value; }
};
...
public:
    ...
    PrimitiveUUID toUUID(ArrayPosition pos) const {
        if (pos.value >= position_to_uuid_.size()) {
            throw std::out_of_range("ArrayPosition " + std::to_string(pos.value) +
                                    " out of range [0, " + std::to_string(position_to_uuid_.size()) + ")");
        }
    ...
    void validatePosition(size_t value, const char* context) const {
        if (value >= mapper_->getPrimitiveCount()) {
            throw std::runtime_error(
                std::string(context) + ": Position " + std::to_string(value) +
                " out of range [0, " + std::to_string(mapper_->getPrimitiveCount()) + ")");
        }
    }
    ...
    void warnPossibleMixup(size_t value, const char* context) const {
        if (value >= mapper_->getPrimitiveCount()) {
            ...
            if (mapper_->isValidUUID(test_uuid)) {
                std::cerr << "WARNING [" << context << "]: Value " << value
                         << " looks like UUID but used as position\n";
            }
        }
    }
};
