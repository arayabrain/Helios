...
void RayTracingGeometry::validate() const {
    ...
    if (primitive_UUIDs.size() != primitive_count) {
        helios_runtime_error("RayTracingGeometry validation failed: primitive_UUIDs.size()=" +
                             std::to_string(primitive_UUIDs.size()) +
    ...
    if (!primitive_UUIDs.empty()) {
        ...
        if (primitive_positions.size() != expected_size) {
            helios_runtime_error("RayTracingGeometry validation failed: primitive_positions.size()=" +
                                 std::to_string(primitive_positions.size()) +
        ...
        for (size_t pos = 0; pos < primitive_UUIDs.size(); pos++) {
            ...
            if (UUID >= primitive_positions.size()) {
                helios_runtime_error("RayTracingGeometry validation failed: primitive_UUIDs[" +
                                     std::to_string(pos) + "]=" + std::to_string(UUID) +
            ...
            if (mapped_pos == UINT_MAX) {
                helios_runtime_error("RayTracingGeometry validation failed: primitive_positions[" +
                                     std::to_string(UUID) + "] == UINT_MAX (should map to position " +
                                     std::to_string(pos) + ")");
            }

            if (mapped_pos != pos) {
                helios_runtime_error("RayTracingGeometry validation failed: primitive_positions[" +
                                     std::to_string(UUID) + "]=" + std::to_string(mapped_pos) +
    ...
    if (patches.UUIDs.size() != patch_count) {
        helios_runtime_error("RayTracingGeometry validation failed: patches.UUIDs.size()=" +
                             std::to_string(patches.UUIDs.size()) +
                             " != patch_count=" + std::to_string(patch_count));
    }

    if (patches.vertices.size() != patch_count * 4) {
        helios_runtime_error("RayTracingGeometry validation failed: patches.vertices.size()=" +
                             std::to_string(patches.vertices.size()) +
                             " != patch_count*4=" + std::to_string(patch_count * 4));
    }

    if (triangles.UUIDs.size() != triangle_count) {
        helios_runtime_error("RayTracingGeometry validation failed: triangles.UUIDs.size()=" +
                             std::to_string(triangles.UUIDs.size()) +
                             " != triangle_count=" + std::to_string(triangle_count));
    }

    if (triangles.vertices.size() != triangle_count * 3) {
        helios_runtime_error("RayTracingGeometry validation failed: triangles.vertices.size()=" +
                             std::to_string(triangles.vertices.size()) +
                             " != triangle_count*3=" + std::to_string(triangle_count * 3));
    }

    if (tiles.UUIDs.size() != tile_count) {
        helios_runtime_error("RayTracingGeometry validation failed: tiles.UUIDs.size()=" +
                             std::to_string(tiles.UUIDs.size()) +
                             " != tile_count=" + std::to_string(tile_count));
    }

    if (tiles.vertices.size() != tile_count * 4) {
        helios_runtime_error("RayTracingGeometry validation failed: tiles.vertices.size()=" +
                             std::to_string(tiles.vertices.size()) +
                             " != tile_count*4=" + std::to_string(tile_count * 4));
    }

    if (voxels.UUIDs.size() != voxel_count) {
        helios_runtime_error("RayTracingGeometry validation failed: voxels.UUIDs.size()=" +
                             std::to_string(voxels.UUIDs.size()) +
                             " != voxel_count=" + std::to_string(voxel_count));
    }

    if (voxels.vertices.size() != voxel_count * 8) {
        helios_runtime_error("RayTracingGeometry validation failed: voxels.vertices.size()=" +
                             std::to_string(voxels.vertices.size()) +
                             " != voxel_count*8=" + std::to_string(voxel_count * 8));
    }

    if (bboxes.UUIDs.size() != bbox_count) {
        helios_runtime_error("RayTracingGeometry validation failed: bboxes.UUIDs.size()=" +
                             std::to_string(bboxes.UUIDs.size()) +
    ...
    if (disk_UUIDs.size() != disk_count) {
        helios_runtime_error("RayTracingGeometry validation failed: disk_UUIDs.size()=" +
                             std::to_string(disk_UUIDs.size()) +
                             " != disk_count=" + std::to_string(disk_count));
    }

    if (disk_centers.size() != disk_count) {
        helios_runtime_error("RayTracingGeometry validation failed: disk_centers.size()=" +
                             std::to_string(disk_centers.size()) +
                             " != disk_count=" + std::to_string(disk_count));
    }

    if (disk_radii.size() != disk_count) {
        helios_runtime_error("RayTracingGeometry validation failed: disk_radii.size()=" +
                             std::to_string(disk_radii.size()) +
                             " != disk_count=" + std::to_string(disk_count));
    }

    if (disk_normals.size() != disk_count) {
        helios_runtime_error("RayTracingGeometry validation failed: disk_normals.size()=" +
                             std::to_string(disk_normals.size()) +
                             " != disk_count=" + std::to_string(disk_count));
    }
    ...
    for (size_t i = 0; i < object_subdivisions.size(); i++) {
        ...
        if (subdiv.x < 1 || subdiv.y < 1) {
            helios_runtime_error("RayTracingGeometry validation failed: object_subdivisions[" +
                                 std::to_string(i) + "] = (" + std::to_string(subdiv.x) + "," +
                                 std::to_string(subdiv.y) +
    ...
    if (subdivision_anomalies > primitive_count / 2) {
        std::cerr << "WARNING [RayTracingGeometry]: " << subdivision_anomalies << " / "
                  << primitive_count << " primitives have subdivisions > (1,1). "
