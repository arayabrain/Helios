...
DOCTEST_TEST_CASE("RadiationModel - Camera Pixel UUID Indexing Validation") {
    ...
    // Check spatial correctness with horizontal flip applied
    // With flip: world-left (ID=1) appears at RIGHT side, world-right (ID=3) appears at LEFT side
    // Sample left region of label map (x=[20,24]) - should contain world-RIGHT patch (ID=3)
    ...
    // Left region of output should see world-RIGHT patch (ID=3) due to horizontal flip
    DOCTEST_CHECK_MESSAGE(right_votes > left_votes,
                          "Left region of output should see world-right patch (ID=3) due to horizontal flip");
    DOCTEST_CHECK_MESSAGE(right_votes > center_votes,
                          "Left region should predominantly see right patch");

    // Sample right region of label map (x=[39,43]) - should contain world-LEFT patch (ID=1)
    ...
    // Right region of output should see world-LEFT patch (ID=1) due to horizontal flip
    DOCTEST_CHECK_MESSAGE(left_votes > right_votes,
                          "Right region of output should see world-left patch (ID=1) due to horizontal flip");
    DOCTEST_CHECK_MESSAGE(left_votes > center_votes,
                          "Right region should predominantly see left patch");
...
DOCTEST_TEST_CASE("RadiationModel - Pixel Labeling with Fine Tessellation") {
    ...
    DOCTEST_CHECK_MESSAGE(valid_percentage > 95.0f,
                          "Pixel labeling with fine tessellation should have >95% valid hits, got "
                          << valid_percentage << "%");
